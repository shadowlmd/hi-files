...
function GetStrCRC( const S: String ) : Longint;
function GetFileCRC( const FileName: String ) : Longint;
...
begin
  ...
  mptr := MapViewOfFile( hMapping, FILE_MAP_READ, 0, 0, 0 );
  if mptr = nil then
  begin
  ...
  crc := $FFFFFFFF;
  p := mptr;
  n := p + GetFileSize( hFile, nil );
  while p < n do
  begin
    crc := crc_32_tab[byte(crc xor longint(p^))] xor ((crc shr 8) and $00FFFFFF);
    Inc( p );
  end;
