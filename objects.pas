...
begin
   ...
   if (IndexSize > 0) then
   begin                                              { We must have strings }
     I := 0;                                          { First entry }
     while (I < IndexSize) and (S = '') do
     begin
       if ((Key - Index^[I].Key) < Index^[I].Count) then  { Diff less than count }
         ReadStr(S, Index^[I].Offset, Key-Index^[I].Key);                        { Read the string }
       Inc(I);                                        { Next entry }
     end;
   end;
...
begin
  ...
  begin
    Writeln('Error put string Key=',Key,'  S='''+S+'''');
    halt(255);
  end;
  {$ENDIF}
