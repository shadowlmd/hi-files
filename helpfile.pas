...
{                                                }
{   Дополнительная функциональность для Hi-Files }
{   (c) 2002 Leemon, [2:461/79]                  }
{                                                }
{************************************************}
...
procedure THelpViewer.PushTrack;
var
  tmp: PTopicTrack;
begin
  New( tmp );
...
procedure THelpViewer.PopTrack;
var
  tmp: PTopicTrack;
begin
  if Track <> nil then
  begin
    Destroy(Topic);
    Topic := HFile^.GetTopic(Track^.Context);
    TopicCtx := Track^.Context;
    Topic^.SetWidth(Size.X);
    SetLimit(Limit.X, Topic^.NumLines);
    ScrollTo(Track^.Scroll.X, Track.Scroll.Y);
    Selected := Track^.SelXref;
    DrawView;
    tmp := Track;
    Track := Track^.Next;
    Dispose( tmp );
